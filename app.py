...
STUDENT_DATA = {
    "1DA25SCS01": "BALAPRIYA F",
    "1DA25SCS02": "BHAVANA A",
    "1DA25SCS03":  "CHARAN A",
    "1DA25SCS04":  "CHETHAN PRASAD L",
    "1DA25SCS05":  "DEEPTHI K",
    "1DA25SCS06":  "DILIP K",
    "1DA25SCS07":  "GURUKIRAN K L",
    "1DA25SCS08":  "HARSHITHA P",
    "1DA25SCS09":  "KUSHI PATIL",
    "1DA25SCS10":  "L TEJASHWINI",
    "1DA25SCS11":  "PRAKASH V",
    "1DA25SCS12":  "RAKSHA M K",
    "1DA25SCS13":  "RAKSHITHA M J",
    "1DA25SCS14":  "RUJULA R",
    "1DA25SCS15":  "SAAKETH D H",
    "1DA25SCS16":  "SHWETA",
    "1DA25SCS17":  "SIBIN SIMON",
    "1DA25SCS18":  "SRIPADA RAO H",
...
def display_header(is_login_page=False):
    ...
    st.markdown(f"""
        <div style="
            ...
            padding: 15px;
            border-radius: 10px;
            border-left: 5px solid {main_title_color};
            margin-bottom: 15px;">
            <h1 style="
            ...
            <div style="
                background-color: {main_title_color};
                color: white;
        ...
        <hr style='border: 1.5px solid {main_title_color}; margin-top: 10px;'>
    """, unsafe_allow_html=True)
...
if not st.session_state.authenticated:
    st.set_page_config(
        page_title="Portal Login | Dr. AIT",
        page_icon="🔐",
        layout="centered"
    )
    ...
    with st.form("Login"):
        ...
        role = st.selectbox("Select Role", ["Faculty", "CR"], index=None, placeholder="Select role")
        ...
        if submit:
            ...
            elif password == CREDENTIALS.get(role, ""):
...
with st.sidebar:
    st.markdown(f"### Welcome, \n**{st.session_state.user_role}**", unsafe_allow_html=True)
...
with tab1:
    ...
    with c1:
        sub = st.selectbox("Subject", list(SUBJECT_INFO.keys()), index=None, placeholder="Select subject")
    with c2:
    ...
    if sub:
        ...
        if st.button("SAVE ATTENDANCE", type="primary", use_container_width=True):
            if None in st.session_state.att_records.values():
                st.warning("Please mark attendance for all students.")
            else:
                st.balloons()
...
with tab2:
    ...
    st.info("Attendance records will appear here once saved to the database.")
